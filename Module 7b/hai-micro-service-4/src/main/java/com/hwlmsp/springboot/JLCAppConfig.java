...
public class JLCAppConfig {
	...
	return ExchangeBuilder.headersExchange("myhello-exchange-4").build(); 
	}
	...
	@Bean 
	Binding helloBinding1(Queue myHelloQueue1, HeadersExchange myHelloExchange) { 
	return BindingBuilder.bind(myHelloQueue1) 
	.to(myHelloExchange) .where("myheader").matches("JLC1"); 
	}
	
	@Bean 
	Binding helloBinding2(Queue myHelloQueue2, HeadersExchange myHelloExchange) { 
	return BindingBuilder.bind(myHelloQueue2) 
	.to(myHelloExchange).where("myheader").matches("JLC2"); 
	}
	
	@Bean 
	Binding helloBinding3(Queue myHelloQueue3, HeadersExchange myHelloExchange) { 
	return BindingBuilder.bind(myHelloQueue3) 
	.to(myHelloExchange).where("myheader").matches("JLC3"); 
	} 
